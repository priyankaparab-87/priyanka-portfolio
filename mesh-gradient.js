// ---------------------------------------------------------------------
// "Mesh Gradient" WebGL background — one independent instance per work
// card. Adapted from Paper Shaders: https://shaders.paper.design/mesh-gradient
// Licensed under Apache-2.0: https://github.com/paper-design/shaders
// Plain WebGL1, no libraries. Fullscreen triangle + the fragment shader
// below, driven by the packed u_scene/u_shape/u_surface/u_finish/
// u_transform/u_space/u_cursor uniforms it expects.
// ---------------------------------------------------------------------
(function () {
  ...
  // Verbatim from the "Mesh Gradient" shader (Paper Shaders, Apache-2.0).
  const FRAGMENT_SRC = `
...
void main() {
  ...
  // Converts "#RRGGBB" to a 0..1 float triple.
  function hexToRgb(hex) {
    const n = parseInt(hex.slice(1), 16);
    return [((n >> 16) & 255) / 255, ((n >> 8) & 255) / 255, (n & 255) / 255];
  }

  // Mounts one independent WebGL1 mesh-gradient instance on the given
  // canvas id. hexColors is 3–8 "#RRGGBB" strings (low → high); seed feeds
  // u_transform.x so each card's field looks distinct even with identical
  // shape/surface/finish dials.
  function mountMeshGradient(canvasId, hexColors, seed) {
    const canvas = document.getElementById(canvasId);
    if (!canvas) return;

    const gl = canvas.getContext('webgl') || canvas.getContext('experimental-webgl');
    if (!gl) {
      console.warn('WebGL unavailable — mesh-gradient background skipped for', canvasId);
      return;
    }
    ...
    // no index buffer, no quad, no gl_VertexID needed for WebGL1).
    ...
    // Recipe colours, padded to 8 vec3s (u_colorCount tells the shader to
    // only use the first hexColors.length of them). Same "feel" dials for
    // every card — only the palette and seed vary per instance.
    const packed = new Float32Array(24);
    for (let i = 0; i < 8; i++) {
      const [r, g, b] = hexToRgb(hexColors[Math.min(i, hexColors.length - 1)]);
      packed[i * 3] = r;
      packed[i * 3 + 1] = g;
      packed[i * 3 + 2] = b;
    }
    gl.uniform3fv(u_colors, packed);
    ...
    // --- Sizing: cap devicePixelRatio at 2, resize with the card. ----------
  ...
  mountMeshGradient('mesh-wismo', ['#120D31', '#6C4CF1', '#EF5DA8', '#FFD166'], 4309.0);
  mountMeshGradient('mesh-customer360', ['#0D1B2A', '#3A86FF', '#8338EC', '#FFBE0B'], 1200.0);
  mountMeshGradient('mesh-recare', ['#1B0620', '#F72585', '#B5179E', '#4CC9F0'], 777.0);
  mountMeshGradient('mesh-cart', ['#1A0800', '#E63946', '#F3722C', '#F9C74F'], 2600.0);
})();
